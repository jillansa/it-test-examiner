...
if ($_POST["idPreguntaClasificacion"] == "0") {
    ...
    $clasificacionSelect = $_POST["clasificacionSelect"];
    ...
    $clasificacionSelect = $_SESSION["clasificacionSelect"];
...
$queryPregunta = "SELECT RAND() as random, p.id, p.texto, pc.id as idPreguntaClasificacion, pc.idClasificacion, c.tema, e.descripcion as examen, e.modalidad,
(select concat(o.descripcion,' ', o.anio) from tabOferta o where e.idOferta = o.id) as oferta, e.fecha_examen, 
...
// FILTRO DE TEMA
// La pregunta esta vinculada a temario elegido
if (isset($tema) && $tema!= null && $tema!= "") {
    $pregunta['metadatos'] = $pregunta['metadatos'] . " | Tema Filter ON ";
    $queryPregunta = $queryPregunta . " AND c.id IN ( 
        -- sea un tema  
        SELECT DISTINCT tema1.id 
        FROM tabClasificacion tema1
        WHERE tema1.id = $tema
        UNION
        -- o un bloque de los temas del cuerpo seleccionado
        SELECT DISTINCT tema2.id 
        FROM tabClasificacion tema2
        WHERE tema2.idBloque = $tema
    )";
} else {
